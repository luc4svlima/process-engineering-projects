...
# Analise de sendibilidade, pode ser usado para literatura ou experimental
import math
import numpy as np
...
kij = -0.10
lista_kij = [-0.10,-0.09,-0.11]     # sensibilidade local
...
plt.scatter(x_exp, T_exp_C, s=88, marker="o",
            edgecolors="none", label="Experimental - líquido")
plt.scatter(y_exp, T_exp_C, s=88, marker="s",
            edgecolors="none", label="Experimental - vapor")

for kij in lista_kij:
    ...
    print(f"kij={kij:.2f} | pontos={len(curva_x)} | falhas={len(falhas)}")
...
plt.title(
    "Diagrama T-x-y água-etanol — Sensibilidade do kij",
    fontsize=18
)
...
plt.savefig(
    "diagrama_Txy_sensibilidade_kij.png",
    dpi=600,
    bbox_inches="tight"
)
